...
const Login = () => {
  ...
  // Handle OAuth callback: ?token=... from Google redirect
  useEffect(() => {
    const token = searchParams.get("token");
    const error = searchParams.get("error");
    if (error) {
      toast.error(error === "google_not_configured" ? "Googleログインは設定されていません。" : "Googleログインに失敗しました。");
  ...
  return (
    <div className="min-h-screen flex flex-col lg:flex-row">
      ...
      <div className="hidden lg:flex lg:w-[52%] relative overflow-hidden bg-gradient-to-br from-[#1e3a5f] via-[#1a3050] to-[#0f1f35] items-center justify-center p-12">
        ...
        {/* Inline SVG decorations: construction-themed abstract icons */}
      ...
      <div className="flex-1 flex items-center justify-center bg-[#f8f7f4] px-6 py-12 lg:px-12 relative overflow-hidden">
        ...
        <div className="w-full max-w-[400px] relative z-10">
          <h2 className="text-2xl font-bold text-[#1e3a5f] text-center mb-1">ログイン</h2>
          <p className="text-sm text-[#6b7a8d] text-center mb-6">アカウントにログインしてください</p>
          ...
          <div className="relative mb-6">
            ...
            <p className="relative text-xs text-[#6b7a8d] text-center bg-[#f8f7f4] px-2">または メールでログイン</p>
          </div>
